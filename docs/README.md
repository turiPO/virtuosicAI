...
OpenAI created two transformer based models as well: first Musenet and then Jukebox. Musnet has similar architecture as GPT-2 and trained in the same way: prediction of a next token in a sequence. Musenet has been trained on MIDIs datasets which where augmented for better results and for expansion of the amount of the training data. [7]



Like many other works, the main disadvantage of Musenet is that it is pure language based training while good music has an important dimension of the performance. Jukebox, by openAI, can produce the whole performance: melody, harmony and even words. 
